...
class ImageCaptionModel(nn.Module):
    ...
    def forward(self, cnn_features, token_ids, is_train):
        ...
        if self.use_attention:
            # Permute [batch, C, H, W] → [batch, H, W, C], then flatten to [batch, num_regions, C]
            cnn_features = cnn_features.permute(0, 2, 3, 1)
            cnn_features = cnn_features.view(cnn_features.size(0), -1, cnn_features.size(-1))
            # Project each region: nn.Linear applies to the last dim → [batch, num_regions, hidden_size]
            processed_img_feat = self.feature_projection(cnn_features)
...
class LSTMCell(nn.Module):
    def __init__(self, input_size: int, hidden_state_size: int):
        ...
        # Forget gate
        self.weight_f = nn.Parameter(torch.randn(n, hidden_state_size) * std)
        self.bias_f   = nn.Parameter(torch.full((1, hidden_state_size), 2.0))
        # Input gate
        self.weight_i = nn.Parameter(torch.randn(n, hidden_state_size) * std)
        self.bias_i   = nn.Parameter(torch.full((1, hidden_state_size), 2.0))
        # Output gate
        self.weight_o = nn.Parameter(torch.randn(n, hidden_state_size) * std)
        self.bias_o   = nn.Parameter(torch.full((1, hidden_state_size), 2.0))
        # Cell candidate
        self.weight   = nn.Parameter(torch.randn(n, hidden_state_size) * std)
    ...
    def forward(self, x, hidden_state):
        ...
        combined = torch.cat([h_old, x], dim=1)  # [batch, hidden_state_size + input_size]

        f = torch.sigmoid(combined @ self.weight_f + self.bias_f)
        i = torch.sigmoid(combined @ self.weight_i + self.bias_i)
        o = torch.sigmoid(combined @ self.weight_o + self.bias_o)
        g = torch.tanh(combined @ self.weight   + self.bias)
...
class Attention(nn.Module):
    def __init__(self, cnn_feature_dim, hidden_dim):
        ...
        self.features_to_hidden = nn.Linear(cnn_feature_dim, hidden_dim)
    ...
    def forward(self, features, hidden_state):
        ...
        """
        hidden_proj = self.hidden_to_hidden(hidden_state).unsqueeze(1)  # [batch, 1, hidden_dim]
        enc_proj = self.features_to_hidden(features)                    # [batch, num_regions, hidden_dim]
        att = self.relu(hidden_proj + enc_proj)                         # [batch, num_regions, hidden_dim]
        att_scores = self.hidden_to_attention_score(att).squeeze(2)     # [batch, num_regions]
